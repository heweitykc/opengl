...
true.
*/
SLbool SLShape::animate(SLfloat timeMS)
{  
   ...
   {  
      ...
      if (dynamic_cast<SLGroup*>(this))
      {  SLNode* current = ((SLGroup*)this)->first();
         while (current)
         {  if (current->animate(timeMS)) gotAnimated = true;
...
{    
   ...
   {  
      ...
      if ((sv->drawBits()->get(SL_DB_AXIS) || _drawBits.get(SL_DB_AXIS)) &&
          !dynamic_cast<SLGroup*>(this))
         _aabb.drawAxisWS();
