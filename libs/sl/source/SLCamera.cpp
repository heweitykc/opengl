...
SLbool SLCamera::shapeUpdate(SLSceneView* sv, SLfloat elapsedTimeMS)
{  
...
{  
   ...
   {  
      ...
      case stereoSideBySideD:
         _fov = sv->oculus()->yFov();
         stateGL->projectionMatrix = sv->oculus()->projection(eye, _clipNear, _clipFar);
         break;
         
      // all other stereo projections
      default: 
         // assymetric frustum shift d
   ...
   {  SLint fbW2 = sv->oculus()->fbHalfWidth();
      SLint fbH  = sv->oculus()->fbHeight();
      if (eye==leftEye) 
           stateGL->viewport(   0, 0, fbW2, fbH);
      else stateGL->viewport(fbW2, 0, fbW2, fbH);
   } else
   if (_projection == stereoSideBySide)
   {  
      if (eye==leftEye) 
           stateGL->viewport( 0, 0, w2, h);
      else stateGL->viewport(w2, 0, w2, h);
   } else
