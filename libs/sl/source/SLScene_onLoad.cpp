...
SLGroup* BuildFigureGroup(SLMaterial* material)
{
   ...
   legLeft = (SLGroup*)figure->getNode("left leg group");
   ...
   legRight = (SLGroup*)figure->getNode("right leg group");
   legRight->rotate(70, 1,0,0);
...
{  
   ...
   if (sceneName == cmdSceneSmallTest)
   {
      ...
      info("Minimal texture mapping example with one light source.");

      // Create textures
      SLGLTexture* texC = new SLGLTexture("earth2048_C.jpg");
      ...
      scene->addNode(new SLRectangle(SLVec2f(-5,-5),SLVec2f(5,5),20,20,"Rect", m1));
      scene->addNode(cam1);
   ...
   if (sceneName == cmdSceneFrustumCull2) //....................................
   {
      ...
      // Define camera
      SLCamera* cam1 = new SLCamera;
      cam1->posAtUp(0,100,180);

      // Floor rectangle
      SLRectangle* rect = new SLRectangle(SLVec2f(-100,-100), SLVec2f(100,100), 
                                          SLVec2f(0,0), SLVec2f(50,50), 50, 50, "Floor", m1);
      rect->rotate(90, -1,0,0);
      rect->translate(0,0,-5.5f);
      ...
      {  for (SLint iX=-size; iX<=size; ++iX)
         {  if (iX!=0 || iZ!=0)
            {
               SLRefGroup* f = new SLRefGroup(figure);
               f->translate(float(iX)*5, 0, float(iZ)*5);
               scene->addNode(f);
            }
         }
      }
   ...
   if (sceneName == cmdSceneRTMuttenzerBox) //.................................
   {
      ...
      //// Material for mirror sphere (orig)
      ...
      SLMaterial* refr=new SLMaterial("refr", blackRGB, blackRGB, 1000, 0.95f, 0.95f, 1.5f);
      refr->translucency(1000);
      refr->transmission(SLCol4f::WHITE);
      refr->textures().push_back(tex1);
      refr->shaderProg(sp2);

      // Material for glass sphere (orig)
