...
{  assert(txtFont);
   ...
   _text = new SLText(text, txtFont, txtColor, 
                      _btnW-2*sv->dpmm()*BTN_BORDER_W_MM);
}
//-----------------------------------------------------------------------------
SLButton::~SLButton()
{  
...
void SLButton::buildBuffers()
{
   ...
   SLfloat 	 w = _btnW;
   SLfloat 	 h = _btnH;
   SLfloat 	 mx = x + 2*sv->dpmm()*BTN_GAP_W_MM; // center x of check mark
...
{  
   ...
   if (_btnW==0.0f) _btnW = _textSize.x + 2*sv->dpmm()*BTN_BORDER_W_MM;
   ...
   {  SLfloat maxW = FLT_MIN;
      ...
      btn = (SLButton*)_first;
      while (btn)
      {  btn->btnW(maxW + 2*sv->dpmm()*BTN_BORDER_W_MM);
...
{  
   ...
   {  // set children Y-pos. according to its parent
      ...
      if (numNodes>2)
         curChildY -= (numNodes/2) * (childH + sv->dpmm()*BTN_GAP_H_MM);
      curChildY = SL_max(curChildY, minMenuPos.y);
             
      // Loop through children & set their position
      SLButton* btn = (SLButton*)_first;
      while (btn)
      {  btn->setPosRec(_minX + _btnW + sv->dpmm()*BTN_GAP_W_MM, curChildY);
         curChildY += btn->btnH() + sv->dpmm()*BTN_GAP_H_MM;
         btn = (SLButton*)btn->next();
      }
   }
}
