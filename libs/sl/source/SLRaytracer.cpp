...
SLbool SLRaytracer::renderClassic()
{
   ...
   for (SLint x=0; x<_img[0].width(); ++x)
   {  for (SLint y=0; y<_img[0].height(); ++y)
      ...
      {  _pcRendered = (SLint)((SLfloat)x/(SLfloat)_img[0].width()*100);
         guiRTWndUpdate();
         t1 = SLScene::current->timeMilliSec();
      }
   }
...
void SLRaytracer::getAAPixels()
{
   ...
   for (SLint x=0; x<_img[0].width(); ++x) gotSampled[x] = false;
      ...
      {  SLuint x = _aaPixels[i].x;
         ...
         {  for (SLint i=0; i<_aaSamples; ++i)
            {  if (i==centerIndex && j==centerIndex) 
               ...
               {  SLVec3f primaryDir(_BL + _pxSize*((xpos+i*f)*_LR + (ypos+j*f)*_LU));
                  primaryDir.normalize();
                  SLRay primaryRay(_EYE, primaryDir, (SLfloat)x, (SLfloat)y);
                  color += traceDistrib(&primaryRay);
               }
